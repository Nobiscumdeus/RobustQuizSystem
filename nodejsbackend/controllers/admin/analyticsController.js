...
exports.analyticsData = async (req, res) => {
  try {
    ...
    const attendanceByExam = buildAttendanceByExam(attendanceRows);
    const participationRate = buildParticipationRate(exams, attendanceByExam);
    const performanceTrends = buildPerformanceTrends(results, timeframe);
    const scoreDistribution = buildScoreDistribution(results);
    const deviceUsage = buildDeviceUsage(results);
