...
import { useCurrentUser } from '../../../hooks/useAuth'; // ADDED
...
const StudentRegistration = () => {
  ...
  // Use the new hook for authentication
  const { isAuthenticated, isLoading: authLoading } = useCurrentUser(); // ADDED
  ...
  useEffect(() => {
    if (!authLoading && !isAuthenticated) {
      navigate("/login", {
        state: { 
          from: "/admin_panel",
          message: "Please login to register students" 
        },
        replace: true,
      });
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Note: With RTK Query and HTTP-only cookies, you shouldn't need to manually pass tokens
      // The cookie will be sent automatically if credentials are included
      const response = await axios.post('http://localhost:5000/students', formData, {
        withCredentials: true, // This sends cookies automatically
        // No need for Authorization header if using HTTP-only cookies
      });
